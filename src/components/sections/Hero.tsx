...
const serviceMap: { [key: string]: string } = {
  ...
  "CRM / ERP": "crm-solutions", // Will scroll to first CRM/ERP service
  ...
  style={{ 
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-40 -right-40 w-[700px] h-[700px] rounded-full" style={{ background: "radial-gradient(circle, rgba(6,79,59,0.06) 0%, transparent 70%)" }} />
        <div className="absolute -bottom-32 -left-32 w-[500px] h-[500px] rounded-full" style={{ background: "radial-gradient(circle, rgba(6,79,59,0.04) 0%, transparent 70%)" }} />
      </div>
      ...
      <motion.div style={{ y, opacity, position: "relative" }} className="relative z-10 max-w-5xl mx-auto px-5 sm:px-8 text-center">
        <motion.div variants={container} initial="hidden" animate="visible" className="flex flex-col items-center gap-8">
          ...
          <motion.div variants={item} className="space-y-2">
            <h1 className="text-5xl sm:text-7xl lg:text-8xl font-black tracking-tight leading-[1.04] font-heading" style={{ color: "#064E3B" }}>
              Engineered for
            </h1>
            <h1 className="text-5xl sm:text-7xl lg:text-8xl font-black tracking-tight leading-[1.04] font-heading gradient-text">
              Digital Impact
            </h1>
            <p className="text-xl sm:text-2xl font-medium mt-4 font-body" style={{ color: "#065F46" }}>
          ...
          <motion.div variants={item} className="flex flex-wrap justify-center gap-2.5">
            {["Websites", "Mobile Apps", "Custom Software", "CRM / ERP", "Billing Software"].map((label) => (
              <motion.button
                key={label}
                onClick={() => navigateToService(label)}
                whileHover={{ scale: 1.05, boxShadow: "0 8px 24px rgba(5, 150, 105, 0.15)" }}
                whileTap={{ scale: 0.98 }}
                className="px-4 py-2 rounded-xl text-sm font-semibold font-body transition-all duration-300 cursor-pointer"
                style={{
          ...
          <motion.div variants={item} className="flex flex-col sm:flex-row items-center gap-4">
            <motion.a href="#contact" onClick={(e) => { e.preventDefault(); document.querySelector("#contact")?.scrollIntoView({ behavior: "smooth" }); }} whileHover={{ scale: 1.05, boxShadow: "0 12px 32px rgba(6, 78, 59, 0.2)" }} whileTap={{ scale: 0.95 }} className="btn-primary" style={{ boxShadow: "0 4px 16px rgba(6, 78, 59, 0.12)", borderRadius: "12px", padding: "12px 24px" }}>
      ...
      {/* Scroll cue */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8 }} className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1.5" style={{ color: "#065F46" }}>
